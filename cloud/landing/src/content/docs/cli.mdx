...
Pass `--pick` to any `search`, `artist`, or `album` call. webfetch renders the candidate table, waits for a keypress (1-9 to select, `q` to quit), and downloads the chosen one to `./` (override with `--out-dir path`). The XMP sidecar is written next to the file.

```bash
webfetch search "sunrise over lake" --pick --out-dir ./assets/
...
With `--json`, output is a JSON array. Each item includes `query`,
`candidateCount`, `top`, and `downloadedPath` when `--download-best` is set.
...
State is persisted at `~/.webfetch/watch/<sha256(query)>.json`. Each tick:

1. Runs the query.
2. Diffs against the last tick by URL.
3. Emits only the new candidates to stdout and, if `--webhook` is set, POSTs them to the URL.

Kill with Ctrl-C. Re-run and state is remembered. Delete the state file to
re-emit everything.
