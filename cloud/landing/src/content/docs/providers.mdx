---
title: Providers
description: The 25 image sources webfetch federates — auth, rate limits, license defaults, and when to enable each one.
section: Core concepts
---

webfetch ships with 25 provider adapters. Each one normalizes a different source into the same `ImageCandidate` shape, tagged with a license and a confidence score. The `DEFAULT_PROVIDERS` set — 19 of the 25 — rejects `UNKNOWN` results by default and prioritizes sources with usable provenance. The other six are opt-in for ToS-grey, paid, managed, or variant use cases.
...
| unsplash | high-quality photography | `UNSPLASH_LICENSE` | `UNSPLASH_ACCESS_KEY` | 1/s demo, 5000/hr prod | no |
...
| youtube-thumb | video thumbnail by id/URL | EDITORIAL_LICENSED | none | 20/s | yes |
...
| managed-browser | hosted browser fallback | UNKNOWN (+heuristic) | cloud entitlement | 0.25/s | yes |
...
|---|---|
| youtube-thumb | You already have YouTube video IDs or URLs and need their thumbnails. |
...
| managed-browser | You're using hosted webfetch and explicitly want managed browser fallback results for review. |
...
Platform licenses, no attribution required. Fastest pipeline of the three.
...
- **Unsplash / Pexels / Pixabay** technically aren't CC0. They use custom platform licenses and are tagged explicitly as `UNSPLASH_LICENSE`, `PEXELS_LICENSE`, and `PIXABAY_LICENSE`.
...
- **Brave** returns web images with no structured license; we upgrade via `heuristicLicenseFromUrl` (Unsplash host → `UNSPLASH_LICENSE`, Commons host → CC-BY-SA pending verification, etc).
...
- **browser / managed-browser** — only runs when explicitly requested. Every result is flagged `viaBrowserFallback: true`; downstream code can route it to review instead of treating it as production-ready.
