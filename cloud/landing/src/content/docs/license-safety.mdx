...
| `UNSPLASH_LICENSE` | Unsplash platform license. | Check platform terms |
| `PEXELS_LICENSE` | Pexels platform license. | Check platform terms |
| `PIXABAY_LICENSE` | Pixabay platform license. | Check platform terms |
...
1. **License rank** — `CC0 (1) < PUBLIC_DOMAIN (2) < CC_BY (3) < CC_BY_SA (4) < UNSPLASH_LICENSE (5) < PEXELS_LICENSE (6) < PIXABAY_LICENSE (7) < EDITORIAL_LICENSED (8) < PRESS_KIT_ALLOWLIST (9) < UNKNOWN (99)`.
...
- `open-only` — only Creative Commons/public-domain candidates are kept.
- `safe-only` (default) — `UNKNOWN` candidates are dropped entirely.
- `context-safe` — explicit alias for `safe-only`.
