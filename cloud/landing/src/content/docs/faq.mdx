...
Apache 2.0. The cloud-hosted product has a separate ToS; the self-hostable core is Apache all the way down. Forks and derivative products are welcome. See [LICENSE](https://github.com/ashlrai/web-fetcher-mcp/blob/main/LICENSE).
...
Open an issue or PR on [GitHub](https://github.com/ashlrai/web-fetcher-mcp). New provider adapters are especially welcome — see [Recipe 10](/docs/cookbook/recipe-10) for the interface. For substantive design changes, open an issue first so we can align on direction before you spend time building.
