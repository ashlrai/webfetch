...
description: Someone sent you a URL. You need to know what license category it falls into. This is that workflow.
section: Cookbook
---

You didn't fetch this image through webfetch. Someone pasted a URL in Slack, or it's in your DAM from years ago, or a partner sent over an asset. Before production use, you need a defensible license claim.
...
4. If not, it runs `heuristicLicenseFromUrl` — Unsplash hosts → `UNSPLASH_LICENSE`, unknown commercial hosts → `UNKNOWN`, etc.
...
The confidence score tells you how much to trust the result. Anything ≥ 0.9 is metadata-backed. Anything ≤ 0.4 is a host heuristic alone — require review before production use.
...
## Automating a reusable-image check
